...
default_lambda = [ 2.6 1.8 1 1.1 ] * M; % the corresponding lambda to use
default_eps = [ 8 6 4 2 ]; % the corresponding epsilon to use
default_K = [ 10 11 11 18]; % the corresponding K to use
...
for trial = 1 : TRIAL
    ...
    for index_N = 1 : N_N
        ...
        U_oracle = 0.5 * (Sigma_s \ c);
        oracle_obj( trial, index_N ) = c' * U_oracle - U_oracle' * Sigma_s * U_oracle;
    end    
end
...
errorbar(log_scan_N, mean(oracle_obj), std(oracle_obj)/sqrt(TRIAL), 'k-', 'MarkerSize', 4 ); hold on; legend_str = [legend_str ; 'ORA'];
xlabel('log(N/M)');
